...
class ReinforceBase():
    ...
    def select_action(self, state, temp=1):
        ...
        m = Multinomial(self.args.vaccine_supply, logits=logits_norm.squeeze() / temp)
        action = m.sample()
...
class ReinforceAtt(ReinforceBase):
    def __init__(self, env, actor_lr, critic_lr, args):

        super(ReinforceAtt, self).__init__(env, args)

        from networks import ActorAtt, CriticAtt

        state = env.reset()

        # create actor and critic network
        self.actor = ActorAtt(state.shape[-1]).to(self.device)
        self.critic = CriticAtt(state.shape[-1], state.shape[-2]).to(self.device)
    ...
    def train(self, states, rewards, log_probs, entropies):
        ...
        log_probs_paths = torch.stack(log_probs_paths)
        rewards_path = torch.tensor(rewards_path, dtype=torch.float32, device=self.device)
        states = torch.tensor(states, device=self.device, dtype=torch.float32)

        # rewards_path = (rewards_path - rewards_path.mean()) / (rewards_path.std() + 1e-8)
        value = self.critic(states.view(-1, states.shape[-2], states.shape[-1]))
        # take a backward step for actor
        entropy_loss = torch.mean(torch.stack(entropies_path))
        actor_loss = -torch.mean(((rewards_path - value.detach().squeeze()) * log_probs_paths) - \
                                 self.args.entropy_coef * entropy_loss
