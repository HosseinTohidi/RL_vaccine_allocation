# -*- coding: utf-8 -*-
"""
Created on Mon Apr  6 23:13:59 2020
...
#        states, rewards, log_probs, entropies, next_states, full_state, done = [], [], [], [], [], [], []
        if arg == 'serial':
            ...
            t0 = time.time()
            results = rollout(envs)
            for i in range(batchSize):
                states, rewards, log_probs, entropies = results[i][0], results[i][1], results[i][2], results[i][3]
                batch_states.append(states)
                batch_rewards.append(rewards)
                batch_log_probs.append(log_probs)
                batch_entropies.append(entropies)    
...
def train_all_v(budget):
    t0 = time.time()
    def helper(i_episode):
        envs = [env(initial_state,0) for i in range(batchSize)]
        threads = []
        batch_states, batch_rewards, batch_log_probs, batch_entropies = [], [], [], []
        results = rollout(envs)
        ...
        print(i_episode)
    # print(f'Episode {i_episode}\t average reward path: {round(avg_raward_path,2)}\t torch mean: {round(torch.mean(value).item(),2)} \telapsed time: {time.time()-t0}')
    results = [helper(i) for i in range(budget)]
    print(time.time()-t0)
...
#train_all_mt(3, arg = 'multi_thread')
print('**********************************')
train_all_mt(10, arg = 'serial')
print('**********************************')
rollout = np.vectorize(rollout, otypes = [object])
#train_all_mt(10, arg = 'vectorized')
#train_one_mt = np.vectorize(train_one_mt)
#results = train_one_mt(np.arange(3))
...
t0 = time.time()
#results = [train_one_mt(i) for i in range(10)]
print(time.time()-t0)
train_all_v(10)
