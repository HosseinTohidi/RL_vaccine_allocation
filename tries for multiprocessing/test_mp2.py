...
if __name__ == "__main__":
    ...
    list_of_objects = [env(initial_state,0) for _ in range(batchSize)]
    t0 = time.time()
    pool = mp.Pool(NUM_CORE)
    list_of_results = pool.map(rollout, ((obj) for obj in list_of_objects))
    pool.close()
    pool.join()
    ...
    print('done')
    t1 = time.time()
    for myenv in list_of_objects:
        result.append(rollout(myenv))
    print(time.time()-t1)        
    
    #t2 = time.perf_counter()
    #results = parallel(delayed(rollout)(obj) for obj in list_of_objects)
