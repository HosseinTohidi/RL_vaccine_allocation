...
plt.plot(-1*np.array(rws[0:7000]), 'b-')
#plt.plot(-1*np.array(torchMean[0:50000]), 'r-' ,label = 'critic value')
plt.ylabel('Avg # of infections (-1 X reward)')
plt.xlabel('batches')
#plt.legend()

dff = pd.DataFrame([rws,torchMean]).T
dff.to_csv('rewardsPLUStorchMean_vlist__age2.csv')
...
chart.set(ylabel ='total # of infections')
